...
- **openai-codex on a ChatGPT account** has entitlement-dependent model access: base GPT includes `gpt-5.5` and `gpt-5.4`. Standalone `-codex` variants (`gpt-5.3-codex`, `gpt-5.2-codex`, `gpt-5.1-codex-max` / `-mini`) return `not supported when using Codex with a ChatGPT account`. Model access beyond that is decided by live provider entitlement: local usage plan labels only rank accounts and never reject a request before transport, and a provider-confirmed entitlement failure surfaces actionable guidance.
