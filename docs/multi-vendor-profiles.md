...
- **openai-codex on a ChatGPT account** has entitlement-dependent model access: base GPT includes `gpt-5.5` and `gpt-5.4`. Standalone `-codex` variants (`gpt-5.3-codex`, `gpt-5.2-codex`, `gpt-5.1-codex-max` / `-mini`) return `not supported when using Codex with a ChatGPT account`. GPT-5.6 Sol access is decided by live provider entitlement: local usage plan labels only rank Sol accounts and never reject a Sol request before transport, and a provider-confirmed entitlement failure surfaces actionable guidance. Spark models retain their existing local filter: if any connected account is confirmed Pro, non-Pro candidates are excluded; if none is confirmed Pro, Plus or unknown-plan candidates may still be attempted.
